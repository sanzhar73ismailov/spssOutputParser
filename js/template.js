...
template.set(
  ...
  template.set(
	"Anova",
	`
 Цель: Оценить влияние показателя "{ЗависимаяПеременная}" на показатель "{НезависимаяПеременная}"
  
 Группы независимые (несвязанные).
 Обоснование выбора метода:
	  Параметр "{{ЗависимаяПеременная}}" является количественным. 
	  Распределение в группах соответствует нормальному.
	  Более двух групп сравнения.
Метод сравнения:  Однофакторный дисперсионный анализ (ANOVA).

Описательная статистика групп:
{% for group in groups %}
В группе "{{group.name}}": 
	  количество наблюдений (n) {{group.n}}, 
	  среднее {meanx}, 
	  стандратная ошибка среднего {{group.ser}}.
{% endfor %}
...
template.set(
	"ChiSquared",
	`
Цель: Оценить влияние показателя "{{var1}}" на показатель "{{var2}}"
...
Корреляция по Спирмену {{notPrefix}}найдена (p = {{correlP}}).
{{ifCorrelFound}}
